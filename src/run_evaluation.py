...
if __name__ == "__main__":
    image_files = np.array([line.split('\n')[0] for line in open("./data/filenames/images.txt")])
    landmark_files = np.array([line.split('\n')[0] for line in open("./data/filenames/landmarks.txt")])

    assert image_files.shape == landmark_files.shape

    options = np.indices(image_files.shape)
    options = options.reshape((-1,))
    n = options.shape[0]

    # random 10% of data for validation
    index_val = np.random.choice(options, int(np.floor(n*0.1)), replace=False)
    with open("./data/filenames/images_val.txt", "w") as f:
        for s in image_files[index_val]:
            f.write(s)
            f.write('\n')

    with open("./data/filenames/landmarks_val.txt", "w") as f:
        for s in landmark_files[index_val]:
            f.write(s)
            f.write('\n')

    options = np.array([i for i in options if i not in index_val])

    # random 10% of data for testing
    index_test = np.random.choice(options, int(np.floor(n * 0.1)), replace=False)
    with open("./data/filenames/images_test.txt", "w") as f:
        for s in image_files[index_test]:
            f.write(s)
            f.write('\n')

    with open("./data/filenames/landmarks_test.txt", "w") as f:
        for s in landmark_files[index_test]:
            f.write(s)
            f.write('\n')

    train_idx = np.array([i for i in options if i not in index_test])
    with open("./data/filenames/images.txt", "w") as f:
        for s in image_files[train_idx]:
            f.write(s)
            f.write('\n')

    with open("./data/filenames/landmarks.txt", "w") as f:
        for s in landmark_files[train_idx]:
            f.write(s)
            f.write('\n')
