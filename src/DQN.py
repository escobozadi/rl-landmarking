...
def get_player(directory=None, files_list=None, landmark_ids=None, viz=False,
    ...
    if task != "train":
        ...
        # is taken care of in expreplay buffer otherwise,
    ...
    if args.task != 'train':
        ...
        if args.val_files is not None:
            ...
            eval_env = get_player(task='eval',
                                  ...
                                  agents=agents,
                                  logger=logger)
        trainer = Trainer(environment, eval_env=eval_env, landmarks=args.landmarks, image_size=IMAGE_SIZE,
                          update_frequency=args.target_update_freq, replay_buffer_size=args.memory_size,
                          init_memory_size=init_memory_size, max_episodes=args.max_episodes,
                          steps_per_episode=args.steps_per_episode, delta=args.delta, batch_size=args.batch_size,
                          gamma=args.discount, number_actions=4, frame_history=FRAME_HISTORY,
